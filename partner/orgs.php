...
// Получение списка организаций для текущего партнера
        ...
        switch ($_POST['action']) {
            case 'get_org':
                ...
                if ($org) {
                    ...
                    echo json_encode(['error' => 'Организация не найдена или нет доступа']);
                }
                break;
                
            case 'update_org':
                $stmt = $pdo->prepare("UPDATE orgs SET 
                    name = ?, inn = ?, kpp = ?, percent = ?, last_updated_by = ?
                    WHERE id = ? AND partner_id = ?");
                
                $stmt->execute([
                    $_POST['name'], $_POST['inn'], $_POST['kpp'], $_POST['percent'], $_SESSION['user_id'],
            ...
            case 'approve_org':
                $stmt = $pdo->prepare("UPDATE orgs SET status = 1, last_updated_by = ? WHERE id = ? AND partner_id = ?");
                $stmt->execute([$_SESSION['user_id'], $_POST['id'], $_SESSION['user_id']]);
                echo json_encode(['success' => true]);
                break;
                
            case 'archive_org':
                $stmt = $pdo->prepare("UPDATE orgs SET status = 3, is_archived = 1, last_updated_by = ? WHERE id = ? AND partner_id = ?");
                $stmt->execute([$_SESSION['user_id'], $_POST['id'], $_SESSION['user_id']]);
                echo json_encode(['success' => true]);
                break;
                
            case 'restore_org':
                $stmt = $pdo->prepare("UPDATE orgs SET status = 2, is_archived = 0, last_updated_by = ? WHERE id = ? AND partner_id = ?");
                $stmt->execute([$_SESSION['user_id'], $_POST['id'], $_SESSION['user_id']]);
                echo json_encode(['success' => true]);
                break;
                
            case 'delete_org':
                // Проверка, используется ли организация в act_detail
                ...
                if ($count > 0) {
                    ...
                    $stmt = $pdo->prepare("DELETE FROM orgs WHERE id = ? AND partner_id = ?");
                    $stmt->execute([$_POST['id'], $_SESSION['user_id']]);
                    echo json_encode(['success' => true]);
                }
                break;
...
<head>
    ...
    <title>Список подчиненных организаций</title>
...
<body>
    <header class="header">
        <div class="header-left">
            ...
            <h1>Список подчиненных организаций</h1>
        </div>
        ...
        <div class="modal-content">
            ...
            <form id="orgForm">
                ...
                <div class="form-actions">
                    ...
                    <button type="button" id="approveBtn" class="btn" style="background: var(--success);">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" style="margin-right: 8px;">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('newOrgBtn').addEventListener('click', function() {
            ...
            document.getElementById('partner_name').value = '<?= htmlspecialchars($_SESSION['user_name'] ?? '') ?>';
        ...
        document.getElementById('approveBtn').addEventListener('click', function() {
            if (confirm('Вы уверены, что хотите отправить организацию на одобрение?')) {
                ...
                .then(data => {
                    ...
                    alert('Организация отправлена на одобрение');
                    location.reload();
                })
                .catch(error => {
