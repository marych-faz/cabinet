        ...
        switch ($_POST['action']) {
            ...
            case 'update_org':
                $stmt = $pdo->prepare("UPDATE orgs SET 
                    name = ?, inn = ?, kpp = ?, percent = ?, partner_id = ?
                    WHERE id = ?");
                
                $stmt->execute([
                    $_POST['name'], $_POST['inn'], $_POST['kpp'], $_POST['percent'], $_POST['partner_id'],
                    $_POST['id']
                ]);
            ...
            case 'archive_org':
                $status = $_POST['is_archived'] ? 3 : 2; // 3 - archived, 2 - blocked
                $stmt = $pdo->prepare("UPDATE orgs SET status = ?, is_archived = ? WHERE id = ?");
                $stmt->execute([$status, $_POST['is_archived'], $_POST['id']]);
                echo json_encode(['success' => true]);
                break;
...
if (!$showArchived) $query .= " WHERE o.is_archived = 0";
if (!empty($search)) $query .= ($showArchived ? " WHERE " : " AND ") . 
                              "(o.name LIKE ? OR o.inn LIKE ? OR o.kpp LIKE ? OR u.name LIKE ?)";
...
// Get partners list for select
$stmt = $pdo->query("SELECT id, name FROM users WHERE is_admin = 0 AND status = 1 AND is_archived = 0");
...
<body>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="status status-<?= $org['status'] ?>">
                        <?= $org['status'] == 0 ? 'Новая' : ($org['status'] == 1 ? 'Подтверждена' : ($org['status'] == 2 ? 'Заблокирована' : 'Архив')) ?>
                    </span></td>
                    <td>
                        ...
                        <button class="action-btn archive-btn" data-id="<?= $org['id'] ?>" data-archived="<?= $org['is_archived'] ?>" title="<?= $org['is_archived'] ? 'Восстановить' : 'В архив' ?>">
                            <?= $org['is_archived'] ? 
            ...
            <form id="orgForm">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="partner_id" class="form-control" required>
                ...
                <div class="form-actions">
                    ...
                    <button type="button" id="archiveBtn" class="btn" style="background: var(--warning);">
                        ...
                        <span id="archiveBtnText">В архив</span>
                    </button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.edit-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                .then(org => {
                    ...
                    document.getElementById('partner_id').value = org.partner_id || '';
                    
                    // Show archive button and set its state
                    document.getElementById('archiveBtn').style.display = 'inline-flex';
                    const isArchived = org.is_archived == 1;
                    document.getElementById('archiveBtnText').textContent = isArchived ? 'Восстановить' : 'В архив';
                    document.getElementById('archiveBtn').setAttribute('data-archived', isArchived ? '1' : '0');
        ...
        document.getElementById('archiveBtn').addEventListener('click', function() {
            const isArchived = this.getAttribute('data-archived') === '1';
            const action = isArchived ? 'восстановить' : 'архивировать';
            
            if (confirm(`Вы уверены, что хотите ${action} организацию?`)) {
                const formData = new URLSearchParams();
                formData.append('action', 'archive_org');
                formData.append('id', document.getElementById('orgId').value);
                formData.append('is_archived', isArchived ? '0' : '1');
                ...
                .then(data => {
                    ...
                    alert(`Организация успешно ${isArchived ? 'восстановлена' : 'архивирована'}`);
                    location.reload();
                })
                .catch(error => {
